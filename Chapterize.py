# -*- coding: utf-8 -*-
"""
Created on Wed Feb  7 23:02:28 2018
...
class Book():
    def __init__(self, filename=None, contents = None, nochapters=False, stats=False, 
                 bible=False, quran=False, meditation = False, buddha = False, gita = False):
        ...
        if contents is None:
            if filename is not None:
                if 'bible' in filename.lower():
                    self.bible = True
                if 'koran' in filename.lower():
                    self.quran = True
                if 'meditations' in filename.lower():
                    self.meditation = True
                if 'gita' in filename.lower():
                    self.gita= True
                if 'buddha' in filename.lower():
                    self.buddha= True
                self.contents = self.getContents()
            else:
                raise ValueError('Either Filename or Contents should be passed')
        else:
        ...
        self.heading_names = self.get_chapter_headings()
...
#        return self.contents.split('\n')
    ...
    def get_chapter_headings(self):
        heading_titles = []
        for i in self.headings[:-1]:
            ...
            if self.buddha:
                ...
                while(True):
                    if j >= len(self.lines)-1:
                        break
                    if (not(bool(not self.lines[j] or self.lines[j].isspace()))):
                        heading_titles.append(self.lines[j])
                        break
                    j = j+1
            elif self.gita:
                j = i+1
                while(True):
                    if j >= len(self.lines)-1:
                        heading_titles.append('Unknown')
                        break
                    if ('HERE' in self.lines[j].strip()):
                        heading_titles.append(re.sub(r'[^\w\s]','',self.lines[j+1].strip().split('"')[1]))
                        break
                    j = j+1
            else:
                heading_titles.append(self.lines[i])
    ...
    def getHeadings(self):
        ...
        ordinalNumberWords = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth', 
                              'seventh', 'eighth', 'ninth','tenth','eleventh', 'twelfth', 'last'] + \
        ...
        #For Quran
#        form5 = 'sura ' + enumerators
        form5 = 'SURA'
        #For Mahabharata
        form6 = 'section ' + enumerators
        #For Bible
        form7 = 'the ' + enumerators + ' book'
        ...
        pat3 = re.compile(form5)
        pat4 = re.compile(form6, re.IGNORECASE)
        pat5 = re.compile(form7, re.IGNORECASE)

        # TODO: can't use .index() since not all lines are unique.

        headings = []
        if self.bible:
            pat_verse_start = re.compile(r"1:1\s", re.IGNORECASE)
            for i, line in enumerate(self.lines):
                if pat_verse_start.match(line.strip()) is not None:
                    j = i-1
                    while(True):
                        if j == 0:
                            headings.append(i)
            ...
            pat_sura= re.compile("SURA")
            for i, line in enumerate(self.lines):
                if pat_sura.match(line.strip()) is not None:
                    headings.append(i)
        elif self.meditation:
            pat_med= re.compile('the ' + ordinalsPat + ' book',re.IGNORECASE)
            for i, line in enumerate(self.lines):
                if pat_med.match(line.strip()) is not None:
                    headings.append(i)
        else:
            for i, line in enumerate(self.lines):
                if pat.match(line.strip()) is not None:
                    headings.append(i)
                if pat2.match(line.strip()) is not None:
                    headings.append(i)
                if pat3.match(line.strip()) is not None:
                    headings.append(i)
                if pat4.match(line.strip()) is not None:
                    headings.append(i)
                if pat5.match(line.strip()) is not None:
                    headings.append(i)
...
#        print (len(self.headingLocations))
#        print (lastHeading)
...
#                print (i)
    ...
    def getStats(self):
        ...
        statsLog = ','.join(stats) + '\n'
        for heading,stat in zip(headings,stats):
            print ("{} : {}".format(heading,stat))
            
        logging.info('Log headings: %s' % headings_text)
        logging.info('Log stats: %s' % statsLog)
#
#        if not os.path.exists('log.txt'):
#            logging.info('Log file does not exist. Creating it.')
#            with open('log.txt', 'w') as f:
#                f.write(headings_text)
#                f.close()
#
